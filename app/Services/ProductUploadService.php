...
class ProductUploadService
{
    ...
    public function store($item, $uploadedBy = 2)
    {
        DB::beginTransaction();
        try {
            $slug = getSlug($item['title'] ?? 'product-' . time());
            
            $data = [
                'title' => $item['title'] ?? null,
                'slug' => $slug,
                'description' => $item['description'] ?? null,
                'accessibility' => $item['accessibility'] ?? 1,
                ...
                'user_id' => auth()->id(),
                'customer_id' => null,
                'file_types' => $item['file_types'] ?? 'jpg'
            ];
            ...
            if (isset($item['thumbnail_image'])) {
                ...
                $upload = $new_file->upload('Product', $item['thumbnail_image'], $product->slug);
